					...
					<form action="<?php echo U('Search/search');?>" method="post">
						<div>
							...
							卖家:<select name="seller" class="input-select" >
								<option value="0">全部</option>
								<option value="1">不按卖家搜索</option>
							</select><br/>
						</div>
						<div>
							搜索内容: <input type="text" name="content" class="input-text" placeholder="如店铺名,标签,卖家">
						</div>
						<div><input type="submit" value="搜索" class="input-submit"></div>
					</form>
					<!-- 搜索结果展示 -->
					<div><?php if(is_array($store)): $i = 0; $__LIST__ = $store;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div style="float:left;width: 100%;padding-bottom: 2em;padding-top: 1em;">
